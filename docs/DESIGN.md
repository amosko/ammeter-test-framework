...
is scheduled at zero, so a "schedule error" there is just the elapsed time. Giving three inconsistent
values is an error rather than a silent choice. On the
